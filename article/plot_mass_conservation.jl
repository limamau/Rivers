...
# Read timeseries
basin_id = 7070307750
timeseries_df = CSV.read("/central/scratch/mdemoura/data/timeseries/timeseries_lv07/basin_$basin_id.csv", DataFrame)
attributes_df = CSV.read("/central/scratch/mdemoura/data/attributes/attributes_lv07/other_attributes.csv", DataFrame)
simulation_df = CSV.read("article/sim_$basin_id.csv", DataFrame)

# Read basin gauge dictionary
basin_gauge_dict = JSON.parsefile("/central/scratch/mdemoura/data/mapping_dicts/gauge_to_basin_dict_lv07_max.json")
...
# Get arrays
runoff1 = (timeseries_df[ts_min_date_idx:ts_max_date_idx, "sro_sum"] + timeseries_df[ts_min_date_idx:ts_max_date_idx, "ssro_sum"]) .* (basin_area*1000000)
obs_streamflow = timeseries_df[ts_min_date_idx:ts_max_date_idx, "streamflow"] .* 24*60*60
lstm_streamflow = simulation_df[sim_min_date_idx:sim_max_date_idx, "sim"] .* 24*60*60
glofas_streamflow = glofas_df[glo_min_date_idx:glo_max_date_idx, "glofas_streamflow"] .* 24*60*60

println("runoff1: ", sum(runoff1))
